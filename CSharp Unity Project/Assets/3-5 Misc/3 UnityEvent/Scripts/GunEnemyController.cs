...
{
    [Tooltip("Idle 状態から Ready 状態に移る時間の最小値（単位: 秒）")]
    [SerializeField] float _minInterval = 1f;
    [Tooltip("Idle 状態から Ready 状態に移る時間の最大値（単位: 秒）")]
    [SerializeField] float _maxInterval = 5f;
    [Tooltip("Ready 状態から攻撃までの時間の最小値（単位: 秒）")]
    [SerializeField] float _minFireTime = 0.1f;
    [Tooltip("Ready 状態から攻撃までの時間の最大値（単位: 秒）")]
    [SerializeField] float _maxFireTime = 0.3f;
    [Tooltip("キャラクターアニメーションのための Animator")]
    [SerializeField] Animator _animator = null;
    [Tooltip("攻撃が当たった時に加算される点")]
